...
def _retention_status(request: Request) -> dict[str, Any]:
    reaper = getattr(request.app.state, "monomer_md_retention_reaper", None)
    if reaper is not None:
        return reaper.snapshot()
    settings = request.app.state.settings
    return {
        "job_retention_enabled": settings.monomer_md_job_retention_enabled,
        "job_retention_days": settings.monomer_md_job_retention_days,
        "job_retention_status": (
            "standby" if settings.monomer_md_job_retention_enabled else "disabled"
        ),
...
async def get_monomer_md_status(request: Request) -> MonomerMdStatusResponse:
    ...
    return _status_response_from_health(
        ...
        retention=_retention_status(request),
    )
...
async def delete_monomer_md_job_artifacts(job_id: str, request: Request) -> MonomerMdJobStatusResponse:
    ...
    if job is None:
        raise HTTPException(status_code=404, detail="Job not found")
    if job["status"] in {"pending", "submitted", "running", "cancel_requested"}:
