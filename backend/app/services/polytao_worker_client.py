...
class PolytaoWorkerError(RuntimeError):
    pass


@dataclass(frozen=True, slots=True)
class PolytaoWorkerSubmission:
...
@dataclass(frozen=True, slots=True)
class PolytaoWorkerSubmitPayload:
    job_id: str
...
class PolytaoWorkerClient:
    def __init__(self, *, base_url: str, timeout_seconds: float) -> None:
        ...
        if parsed.scheme == "http+unix" and parsed.netloc:
            ...
            raise PolytaoWorkerError("POLYTAO_WORKER_BASE_URL must be an http(s) or http+unix URL")
    ...
    def get_health(self) -> dict[str, Any]:
        try:
            ...
            raise PolytaoWorkerError("PolyTAO worker is not reachable") from exc

        if response.status_code >= 400:
            detail = _safe_response_detail(response)
            raise PolytaoWorkerError(f"PolyTAO worker health check failed: {detail}")
        return _response_json_object(response, "health check")

    def submit_job(self, payload: PolytaoWorkerSubmitPayload) -> PolytaoWorkerSubmission:
        try:
        ...
        if response.status_code >= 400:
            detail = _safe_response_detail(response)
            raise PolytaoWorkerError(f"PolyTAO worker rejected the job: {detail}")
        ...
        if worker_job_id is None:
            raise PolytaoWorkerError("PolyTAO worker accepted the job without returning a job id")
        return PolytaoWorkerSubmission(
...
class _UnixSocketWorkerSession:
    ...
    def post(self, url: str, *, json: dict[str, object], timeout: float) -> requests.Response:
    ...
    def _request(
        ...
        *,
        ...
        request_headers = {"Host": "polytao-worker"}
...
def _response_json_object(response: requests.Response, context: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise PolytaoWorkerError(f"PolyTAO worker returned invalid JSON for {context}") from exc
    if not isinstance(data, dict):
        raise PolytaoWorkerError(f"PolyTAO worker returned a non-object JSON payload for {context}")
    return data
