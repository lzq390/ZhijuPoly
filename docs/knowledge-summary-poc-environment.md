...
功能已完成正式后端集成和 9021 页面验收，当前准备最后一次提交与 PR。采集范围为本地知识库检索/文献阅读/反应信息，以及数据库筛选/测量详情/SMILES；公共按钮支持跨模块记录、三段式总结和面板收起后重新查看。
...
- Provider 位于 App 根部，公共总结按钮位于 AppShell 内容区上方，页面切换不会卸载记录状态。在本地知识库或数据库筛选点击“开始记录”即可开启；输入框外 adad 保留兼容。其它模块保留按钮和已有记录，但不增加采集范围。
