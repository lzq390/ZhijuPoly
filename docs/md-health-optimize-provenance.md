...
| `9d2a221` | Runtime probes are bounded and child cancellation terminates the owned process group. | `runtime_probe.py`, `process_control.create_fenced_subprocess_exec`, `process_control.wait_for_process_group`, `runner.py`, and their regression tests; the configuration-agnostic host executable remains selected by the governed launcher. |
