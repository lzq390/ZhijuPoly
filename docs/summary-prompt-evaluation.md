...
本文记录 2026-09-09 的固定样本提示词评测：在同一份操作和内容输入上调整提示词，改善阅读回顾的重点与连贯性；该轮未调整前端展示。模型名称、耗时与测试数量均为当时结果，不代表当前部署配置。后续已完成模型切换、页面优化与正式集成，最终用户验收见[部署与环境文档](knowledge-summary-poc-environment.md)。
...
该轮 v3 两次仍偏长，开头和句式也有波动，仅凭这两次回放不能判定最终效果。真实模型回放均使用 180 秒评测等待；应用默认仍是 60 秒，v3 一次超过、一次低于该限制。当时尚未完成新模型的页面验收；后续模型切换、页面优化及 9021 手动验收已完成，不再将该历史待办作为当前阻塞项。模型输出与响应时间仍会随输入和服务状态变化，调用失败时记录保留并可重试。
