...
来源为 `/data/lzq/gith/SMiPoly/classified_monomers.csv`。按原文件顺序选择前 120 个通过当前 RDKit 标准化且不重复的结构；二胺同时要求 `diamin=true` 和 `pridiamin=true`，二酐要求 `dicAnhyd=true`，并使用当前 SMiPoly 引擎重新确认分类。
