      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 max-h-[430px] space-y-2 overflow-auto pr-1">{dft.history?.items.length ? dft.history.items.map((item) => <button key={item.job_id} type="button" onClick={() => dft.loadJob(item.job_id)} className={cn("w-full rounded-lg border p-3 text-left hover:bg-slate-50", dft.job?.job_id === item.job_id ? "border-sky-300 bg-sky-50" : "border-slate-100 bg-white")}><div className="flex items-center justify-between gap-2"><span className="truncate font-mono text-[11px] text-slate-600">{item.job_id}</span><StatusBadge status={item.status} /></div><div className="mt-2 truncate text-xs font-medium text-slate-800">{item.request.input.smiles}</div><div className="mt-1 text-[11px] text-slate-500">{item.request.calculation_type === "single_point" ? "单点" : "优化"} · {item.request.model} · {formatDate(item.created_at)}</div></button>) : <div className="rounded-lg border border-dashed border-slate-200 p-5 text-center text-xs text-slate-500">{dft.isHistoryLoading ? "读取历史…" : "没有符合条件的任务"}</div>}</div>
