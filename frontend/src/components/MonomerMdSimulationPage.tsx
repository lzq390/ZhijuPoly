          ...
          <div className="flex min-w-0 flex-col gap-4">
            ...
            {simulation.runMode === "formal" ? (
              <section className="rounded-[14px] border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-slate-500"><Button type="button" variant="outline" className="h-8 w-8 rounded-md p-0" disabled={(simulation.historyQuery.page ?? 1) <= 1} onClick={() => simulation.changeHistoryQuery({ page: (simulation.historyQuery.page ?? 1) - 1 })}><ChevronLeft className="h-3.5 w-3.5" /></Button><span>第 {simulation.historyQuery.page ?? 1} / {historyPageCount} 页 · 共 {simulation.history?.total ?? 0} 项</span><Button type="button" variant="outline" className="h-8 w-8 rounded-md p-0" disabled={(simulation.historyQuery.page ?? 1) >= historyPageCount} onClick={() => simulation.changeHistoryQuery({ page: (simulation.historyQuery.page ?? 1) + 1 })}><ChevronRight className="h-3.5 w-3.5" /></Button></div>
              </section>
            ) : null}
