    ...
    return (
      <Card className="overflow-hidden rounded-[28px] border-destructive/20 shadow-none">
        <CardHeader className="min-h-[112px] border-b border-destructive/10 bg-destructiveForeground">
          ...
          <CardDescription>预测请求未完成。请检查结构输入、性能选择或服务状态。</CardDescription>
        </CardHeader>
    ...
    return (
      <Card className="overflow-hidden rounded-[28px] border-white/70 shadow-none">
        <CardHeader className="min-h-[112px] border-b border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(244,248,249,0.88)_100%)]">
          <CardTitle className="text-xl">预测结果</CardTitle>
          <CardDescription>正在计算所选性能。</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 pt-5">
          <EmptyPanel
            loading
            title="正在预测"
            description="预测完成后，结果卡片会在这里刷新。"
          />
        </CardContent>
      </Card>
    );
  ...
  if (!data) {
    return (
      <Card className="overflow-hidden rounded-[28px] border-white/70 shadow-none">
        <CardHeader className="min-h-[112px] border-b border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(244,248,249,0.88)_100%)]">
          <CardTitle className="text-xl">预测结果</CardTitle>
          <CardDescription>暂无预测结果。</CardDescription>
        </CardHeader>
        <CardContent className="pt-5">
          <EmptyPanel
            title="预测面板已就绪"
            description="切换到预测模式，选择性能并提交后，可查看预测值和单位。"
          />
        </CardContent>
      </Card>
    );
      ...
      <CardHeader className="min-h-[120px] gap-4 border-b border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(244,248,249,0.88)_100%)]">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div className="space-y-2">
            ...
            <CardTitle className="text-[1.4rem] tracking-tight">预测结果</CardTitle>
            <CardDescription>所选性能的预测值会以卡片形式展示，便于快速比较。</CardDescription>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge>
              <Sparkles className="mr-1 h-3.5 w-3.5" />
              {`${predictionEntries.length} 项性能`}
            </Badge>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="pt-5">
        <div className="grid gap-4 md:grid-cols-2">
          {predictionEntries.map(({ property, label, unit, value }) => (
            ...
            >
              <div className="text-xs font-medium uppercase tracking-[0.18em] text-teal-700/80">预测</div>
