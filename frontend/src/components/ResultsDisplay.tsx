...
function EmptyState({
  ...
  return (
    <div className="flex min-h-[320px] flex-col items-center justify-center rounded-[24px] border border-dashed border-slate-200 bg-slate-50/80 px-6 py-12 text-center">
...
function SummaryMetric({
  ...
  return (
    <div className="flex min-h-[144px] flex-col rounded-[20px] border border-slate-200 bg-white p-4">
...
export function ResultsDisplay({
  ...
  if (error) {
    return (
      <Card className="overflow-hidden rounded-[24px] border-destructive/20 shadow-none">
        <CardHeader className="min-h-[108px] border-b border-destructive/10 bg-destructiveForeground">
  ...
  if (isLoading) {
    return (
      <Card className="overflow-hidden rounded-[24px] shadow-none">
        <CardHeader className="min-h-[108px] border-b border-slate-200/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)]">
          <CardTitle className="text-xl">Query Results</CardTitle>
          <CardDescription>正在执行查询并汇总属性数据。</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4 pt-5">
          <div className="flex items-center gap-3 rounded-[20px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
            ...
            {[0, 1].map((index) => (
              ...
              >
                <div className="grid auto-rows-fr gap-3 md:grid-cols-2 xl:grid-cols-4">
                  {[0, 1, 2, 3].map((item) => (
                    <div key={item} className="h-[144px] animate-pulse rounded-[18px] bg-slate-100" />
                  ))}
                </div>
                <div className="grid gap-4 xl:grid-cols-3">
                  {[0, 1, 2, 3, 4, 5].map((item) => (
                    <div key={item} className="h-40 animate-pulse rounded-[18px] bg-slate-50" />
                  ))}
                </div>
              </div>
            ))}
  ...
  return (
    <Card className="overflow-hidden rounded-[24px] shadow-none">
      <CardHeader className="min-h-[108px] gap-4 border-b border-slate-200/80 bg-[linear-gradient(180deg,#ffffff_0%,#f8fafc_100%)]">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Badge className="bg-slate-900/5 text-slate-700">{`${data.total} 条结果`}</Badge>
            <Badge className="bg-slate-900/5 text-slate-700">{`${data.query_time_ms.toFixed(1)} ms`}</Badge>
          </div>
        </div>

        <div className="grid auto-rows-fr gap-3 md:grid-cols-2 xl:grid-cols-4">
          <SummaryMetric
            icon={<ScanSearch className="h-4 w-4 text-blue-600" />}
            label="Match Mode"
