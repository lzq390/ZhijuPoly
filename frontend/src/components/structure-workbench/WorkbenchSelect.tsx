...
export type WorkbenchSelectOption<T extends string> = {
  value: T;
  label: string;
  description?: string;
  disabled?: boolean;
...
type WorkbenchSelectProps<T extends string> = {
  id: string;
  value: T;
  options: Array<WorkbenchSelectOption<T>>;
...
export function WorkbenchSelect<T extends string>({
  ...
  return (
    <div ref={rootRef} className={`np-sw-select${open ? " is-open" : ""}${opensUp ? " opens-up" : ""}`}>
      <button
        ref={triggerRef}
        id={id}
        type="button"
        className="np-sw-select__trigger"
        role="combobox"
