...
export const highThroughputDemoScenario: HighThroughputDemoScenario = {
  ...
  agents: [
    {
      ...
      currentBestId: "PI-530",
      bestValue: 296,
      topCandidateIds: ["PI-530", "PI-1973", "PI-699"],
      explorationCandidateIds: ["PI-259", "PI-532", "PI-1247", "PI-2326"],
      recommendation: "蓝色样本锁定右上 Tg 热点，并在左右边界补充外围点，用于确认耐热窗口是否连续。",
      statusByStage: ["待配置", "候选已生成", "定位热点", "闭环迭代中", "输出 p1", "进入配方池", "已解释"],
    ...
    },
    {
      ...
      currentBestId: "PI-708",
      bestValue: 30,
      topCandidateIds: ["PI-708", "PI-1121", "PI-029"],
      explorationCandidateIds: ["PI-015", "PI-106", "PI-1072", "PI-281"],
      recommendation: "绿色样本围绕左侧低 CTE 区域，并在上下边界放置外围点，检查尺寸稳定区的有效边界。",
      statusByStage: ["待配置", "候选已生成", "定位低值区", "闭环迭代中", "输出 p2", "进入配方池", "已解释"],
    ...
    },
    {
      ...
      currentBestId: "PI-124",
      bestValue: 22,
      topCandidateIds: ["PI-124", "PI-099", "PI-104"],
      explorationCandidateIds: ["PI-339", "PI-1069", "PI-1784", "PI-254"],
      recommendation: "紫色样本覆盖左下高延展区域，并向左右两侧扩展，用于判断柔性桥连簇是否外延。",
      statusByStage: ["待配置", "候选已生成", "定位韧性区", "闭环迭代中", "输出 p3", "进入配方池", "已解释"],
    ...
    },
    {
      ...
      currentBestId: "PI-370",
      bestValue: 3.3,
      topCandidateIds: ["PI-370", "PI-734", "PI-834"],
      explorationCandidateIds: ["PI-084", "PI-478", "PI-122", "PI-1016"],
      recommendation: "橙色样本锁定右下高模量区域，同时向左侧链段簇布点，验证刚性提升的可转移性。",
      statusByStage: ["待配置", "候选已生成", "定位刚性区", "闭环迭代中", "输出 p4", "进入配方池", "已解释"],
  ...
  batches: [
    {
      round: 1,
      testedIds: [
        "PI-259", "PI-532", "PI-015", "PI-106",
        "PI-339", "PI-1069", "PI-084", "PI-478",
      ],
      recommendedIds: ["PI-1973", "PI-699", "PI-1121", "PI-029", "PI-099", "PI-104", "PI-734", "PI-834"],
      explanation: "第一轮覆盖式采样回流，建立各性质 Agent 的初始判断。",
    },
    {
      round: 2,
      testedIds: ["PI-1973", "PI-699", "PI-1121", "PI-029", "PI-099", "PI-104", "PI-734", "PI-834"],
      recommendedIds: ["PI-1247", "PI-2326", "PI-1072", "PI-281", "PI-1784", "PI-254", "PI-122", "PI-1016"],
      explanation: "第二轮根据回流结果向热点和外围边界追加验证点。",
    },
    {
      round: 3,
      testedIds: ["PI-1247", "PI-2326", "PI-1072", "PI-281", "PI-1784", "PI-254", "PI-122", "PI-1016"],
      recommendedIds: ["PI-530", "PI-708", "PI-124", "PI-370"],
  ...
  stages: [
    ...
    },
    {
      id: "S3",
      label: "Agent 迭代",
      title: "展示 Agent 回流、推荐、验证的多轮迭代",
      body: "四个 Agent 并行优化各自目标。地图同时保留历史回流样本、当前轮验证样本和下一轮推荐样本，展示从实验回流到区域扩展再到候选收敛的闭环过程。",
      callout: "S3 的重点是迭代过程：回流更新判断，Agent 生成推荐，推荐样本再进入下一轮验证。",
