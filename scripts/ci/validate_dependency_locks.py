...
def validate_backend_split(root: Path) -> list[str]:
    ...
    protected_packages = BASE_RUNTIME_PACKAGES | LEGACY_CUDA_PACKAGES
    legacy_input = {
        package for package in runtime_names if re.fullmatch(r"nvidia-[a-z0-9-]+-cu11", package)
    }
    forbidden_input = (
        runtime_names & (protected_packages | {"pytest", "pytest-asyncio"})
    ) | legacy_input
    if forbidden_input:
        failures.append(
    ...
    legacy_lock = {
        package for package in app_versions if re.fullmatch(r"nvidia-[a-z0-9-]+-cu11", package)
    }
    forbidden_lock = (
        set(app_versions) & (protected_packages | {"pytest", "pytest-asyncio"})
    ) | legacy_lock
    if forbidden_lock:
        failures.append(
...
def validate_dft_worker_locks(root: Path) -> list[str]:
    failures: list[str] = []
    worker = root / "workers" / "monomer_dft_worker"
    lock_paths = (
        worker / "requirements.lock",
        worker / "requirements-ci.lock",
        worker / "build-requirements.lock",
    )
    for path in lock_paths:
        ...
        if "This file was autogenerated by uv" not in text:
            failures.append(f"{path}: missing uv provenance header")
        if "uv 0.11.21; Python 3.12; Linux x86_64" not in text:
            failures.append(f"{path}: provenance must bind uv 0.11.21, Python 3.12, and Linux x86_64")

    runtime_path = worker / "requirements.lock"
    ci_path = worker / "requirements-ci.lock"
    try:
        runtime_versions = versions(runtime_path)
        ci_versions = versions(ci_path)
    except (OSError, ValueError) as exc:
        failures.append(str(exc))
        return failures
    for package, expected in DFT_WORKER_RUNTIME_VERSIONS.items():
        if runtime_versions.get(package) != expected:
            failures.append(f"{runtime_path}: expected {package}=={expected}")
    forbidden_ci = set(ci_versions) & (
        BASE_RUNTIME_PACKAGES
        | LEGACY_CUDA_PACKAGES
        | {"torch", "warp-lang", "nvalchemi-toolkit-ops"}
    )
    if forbidden_ci:
        failures.append(
            f"{ci_path}: CPU-only Worker CI lock contains GPU runtime packages: "
            f"{', '.join(sorted(forbidden_ci))}"
        )
    return failures
...
def validate_tooling(root: Path) -> list[str]:
    ...
    for package in sorted(BASE_RUNTIME_PACKAGES - {"torch", "torchvision", "triton"}):
        if package not in compile_text:
            failures.append(f"scripts/compile_requirements.sh is missing base runtime package: {package}")
    for legacy_marker in ("cu118", "cuda11.8", "-cu11"):
        if legacy_marker in compile_text or legacy_marker in dockerfile:
            failures.append(f"Backend immutable runtime tooling contains legacy CUDA marker: {legacy_marker}")
    return failures
...
def validate(root: Path = REPOSITORY_ROOT) -> list[str]:
    ...
    failures.extend(validate_dft_worker_locks(root))
